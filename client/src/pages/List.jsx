...
const List = () => {
    ...
    useEffect(() => {
        ...
        TokenExpiration();
        axios.post("/videos/get_video_list", {
            ...
            Toast.show("問題が発生しました。後でもう一度お試しください。", {status:'error'});
        });
    ...
    return (
        <div>
            <div className='grid justify-center'>
                <div className={`${ width > 1280 ? 'grid-cols-3' : 'grid-cols-2'} grid gap-10 mt-20 h-[200px] overscroll-auto`}>
                    {
                        data.length === 0 ? '' :
                        data.map((el, index) => {
                            return (
                                <div key={index} className='grid grid-flow-row justify-items-center z-10'>
                                    {/* <VideoPlayer videoId={videoId} num={4} /> */}
                                    <div className='rounded-xl cursor-pointer' onClick={() => handleClick(el.vimeoID)}>
                                        <Thumbnail param={el.vimeoID} width={width > 1280 ? sWidth: xWidth}></Thumbnail>
