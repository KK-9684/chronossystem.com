...
const Study = () => {
    ...
    const handleClick = async (param) => {
        try {
            ...
            const res =  await axios.post("/pdfs/get_pdf", {
            ...
            <div className='grid justify-center items-center h-[100vh]'>
                <div className='h-[300px] grid grid-cols-2 gap-10'>
                    <div className='grid grid-flow-row xl:w-[450px] sm:w-[330px]'>
                        <button className='lesson-btn text-2xl font-bold xl:h-[222px] sm:h-[163px] shadow-lg rounded-3xl' onClick={() => handleClick('1')}>
                            <div className='grid grid-flow-row -mt-5'>
                                <span className='text-3xl'>演習システム</span>
                            </div>
                        </button>
                        <div className='text-center'>
                            <span>段階ヒントつきの解説システムです。１周目を解くときなどはこちら！</span>
                        </div>
                    </div>
                    <div className='grid grid-flow-row xl:w-[450px] sm:w-[330px]'>
                        <button className='lesson-btn text-2xl font-bold xl:h-[220px] sm:h-[163px] shadow-lg rounded-3xl' onClick={() => handleClick('2')}>
                            <div className='grid grid-flow-row -mt-5'>
                                <span className='text-3xl'>復習システム</span>
                            </div>
                        </button>
                        <div className='text-center'>
                            <span>ヒントなし・重要問題のみで、最短時間で復習できるシステムです。</span>
                        </div>
                    </div>
                </div>
            </div>
