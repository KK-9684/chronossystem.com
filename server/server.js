...
io.on('connection', (socket) => {    
    ...
    socket.on('register', (param) => {
        ...
        ChatUser.findOne({name}).then((user) => {
            if (!user) {
                const newUser = new ChatUser({ name, socket_id:socket.id });
                newUser.save();
            } else {
                const updateUser = ChatUser.findByIdAndUpdate(user._id, {socket_id:socket.id });
            }
    ...
    socket.on('more data', (data) => {
        ...
        Chat.find({ $or: [{ sender: name }, { receiver: name }] }).sort({ created: -1 }).skip(count).limit(10).exec()
        .then((data)=>{
    ...
    socket.on('chat message', (data) => {
        ...
        const newMessage = new Chat({ sender, receiver, message, image: image ? imageName : "" });
        if (image) {
            fs.writeFile(process.env.BASE_IMAGE_PATH + imageName, image, (error) => {
            });
        }
        ...
        newMessage.save().then((savedMessage) => {
            if (savedMessage) {
                ...
                ChatUser.findOne({name:receiver}).exec().then((user)=>{
                    if (user) {
