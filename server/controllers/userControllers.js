...
const createUser = async (req, res) => {
    try {
        ...
        if (is_exist) {
            ...
            return res.status(200).send({data:newUser, status: "success", message: "Member add successful!" });
        }
    } catch (err) {
...
const getUser = async (req, res) => {
    try {
        ...
        const result = await User.find({ $or: [{ level: 'user' }, { level: 'manager' }] })
            .skip(skip)
            .limit(limit)
            .sort({ [_sort]: _order });
...
const deleteUser = async (req, res) => {
    try {
        ...
        await Chat.deleteMany({ $or: [{ sender: deletedUser.name }, { receiver: deletedUser.name }] });
...
const sendResetLink = async (req, res) => {
    try {
        ...
        return res.status(500).json({ msg: "問題が発生しました。後でもう一度お試しください。" });
    }
}
